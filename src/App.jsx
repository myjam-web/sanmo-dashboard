...
const dateToWd=(ds)=>{
  try {
    const s=normDate(ds); if(!s) return TDX;
    const idx=WD.findIndex(d=>dKey(d)===s);
    return idx>=0?idx:TDX;
  } catch(e){return TDX;}
};
const dateToWdEnd=(ds)=>{
  // endDate 포함 마지막 평일 인덱스 (inclusive→exclusive)
  try {
    const s=normDate(ds); if(!s) return TDX+1;
    let last=TDX;
    for(let i=0;i<WD.length;i++){if(dKey(WD[i])<=s) last=i;}
    return last+1;
  ...
  return (Array.isArray(raw)?raw:[]).map(s=>{
    try{
      return autoStatus({
        ...
        serviceStart:normDate(s.serviceStart||s.actualBirth||''), // ★ actualBirth→serviceStart 호환
...
export default function App(){
  ...
  const [unavail,   setUnavail]   =useState([]);  // ★ 근무불가
  const [loadState, setLoadState] =useState('loading');
  const [saveState, setSaveState] =useState('idle');
  const [errMsg,    setErrMsg]    =useState('');
  const [warnMsg,   setWarnMsg]   =useState(''); // ★ 경고 토스트
  ...
  const [newWait,   setNewWait]   =useState({name:'',expectedBirth:'',serviceStart:'',durWd:10,memo:''});
  ...
  const [addUnavailWid,setAddUnavailWid]=useState(null); // ★ 근무불가 추가 모달
  ...
  const unavailMap=useMemo(()=>{
    ...
    unavail.forEach(u=>{
      try{
        if(!u||!u.wid) return;
        const startWd=dateToWd(u.startDate);
        const endWd=dateToWdEnd(u.endDate);
  ...
  const checkUnavailConflict=(wid,startWd,durWd)=>{
    const list=unavailMap[wid]||[];
    return list.some(u=>u.startWd<startWd+durWd&&u.endWd>startWd);
  };
  ...
  // ★ 과거 일정 드래그 차단
  const onBlockDown=(e,sch)=>{
    const isPast=(sch.startWd||0)+(sch.durWd||0)<=TDX;
    if(isPast) return; // 과거 일정 클릭은 선택만
    e.stopPropagation();
  ...
  const onRowDrop=(e,wid)=>{
    ...
    setSch(p=>[...p,newSch]);
    setWait(p=>p.filter(w=>w&&w.id!==dragWait));
    setDragWait(null);
    // ★ 소프트 차단 경고
    if(checkUnavailConflict(wid,sw,wc.durWd||10)){
      setWarnMsg('⚠️ 해당 기간에 근무불가 일정이 있습니다! 확인해 주세요.');
  ...
  const doAddUnavail=()=>{
    ...
    setNewUnavail({startDate:'',endDate:'',reason:''});
    setAddUnavailWid(null);
  };
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',height:'100vh',fontFamily:"'Apple SD Gothic Neo','Malgun Gothic',sans-serif",background:'#f1f5f9',color:'#1e293b',overflow:'hidden'}}
      ...
      {/* ★ 경고 토스트 */}
      {warnMsg&&(
        <div style={{position:'fixed',top:50,left:'50%',transform:'translateX(-50%)',background:'#fef3c7',border:'2px solid #f59e0b',borderRadius:10,padding:'10px 18px',fontSize:12,fontWeight:700,color:'#92400e',zIndex:3000,boxShadow:'0 4px 16px rgba(0,0,0,0.15)',display:'flex',alignItems:'center',gap:8}}>
          {warnMsg}
          <button onClick={()=>setWarnMsg('')} style={{border:'none',background:'none',cursor:'pointer',color:'#92400e',fontSize:16,padding:0,lineHeight:1}}>×</button>
        </div>
      )}
      ...
      <header style={{display:'flex',alignItems:'center',gap:8,padding:'0 12px',height:40,background:'#fff',borderBottom:'1px solid #e2e8f0',flexShrink:0,zIndex:20,boxShadow:'0 1px 4px rgba(0,0,0,0.06)'}}>
        <button onClick={()=>setPanelOpen(v=>!v)} title={panelOpen?'패널 닫기':'패널 열기'}
      ...
      <div style={{display:'flex',flex:1,overflow:'hidden'}}>
        ...
        <div style={{width:panelOpen?PANEL_W:0,flexShrink:0,display:'flex',flexDirection:'column',background:'#fff',borderRight:panelOpen?'1px solid #e2e8f0':'none',overflow:'hidden',transition:'width 0.2s ease'}}>
          ...
          <div style={{flexShrink:0,maxHeight:selSch?580:0,overflow:'hidden',transition:'max-height 0.25s ease',borderBottom:selSch?'2px solid #e2e8f0':'none'}}>
            {selSch&&(
              <div className="scroll-y" style={{padding:'10px 12px',display:'flex',flexDirection:'column',gap:7,maxHeight:580}}>
                ...
                <div style={{display:'flex',gap:4}}>
                  {[['expectedBirth','출산예정일'],['serviceStart','서비스시작일']].map(([f,lbl])=>(
          ...
          <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',minHeight:0}}>
            ...
            {addWaitOpen&&(
              <div style={{padding:'7px 10px',background:'#fffbeb',borderBottom:'1px solid #fde68a',display:'flex',flexDirection:'column',gap:4,flexShrink:0}}>
                ...
                <div style={{display:'flex',gap:4}}>
                  <select value={newWait.durWd} onChange={e=>setNewWait(p=>({...p,durWd:+e.target.value}))} style={{width:64,padding:'3px 1px',border:'1px solid #fde68a',borderRadius:5,fontSize:10}}>
                    {[10,15,20].map(n=><option key={n} value={n}>{n}일</option>)}
                  </select>
                  <input placeholder="메모" value={newWait.memo} onChange={e=>setNewWait(p=>({...p,memo:e.target.value}))} style={{flex:1,padding:'3px 6px',border:'1px solid #fde68a',borderRadius:5,fontSize:11}}/>
                </div>
                <div style={{display:'flex',gap:4}}>
                  <button onClick={()=>{if(!newWait.name.trim())return;setWait(p=>[...p,{id:_uid++,...newWait}]);setNewWait({name:'',expectedBirth:'',serviceStart:'',durWd:10,memo:''});setAddWaitOpen(false);}} style={{flex:1,padding:'4px',borderRadius:5,fontSize:11,fontWeight:700,background:'#f59e0b',color:'#fff',border:'none',cursor:'pointer'}}>저장</button>
        ...
        <div ref={wrapRef} style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',minWidth:0}}>
          ...
          <div style={{display:'flex',flex:1,overflow:'hidden',minHeight:0}}>
            ...
            {/* 간트 본문 */}
            <div ref={ganttRef} onScroll={onGScroll} className="scroll-xy" style={{flex:1,position:'relative',cursor:dragging?'grabbing':'default'}}>
                      ...
                      <div style={{height:ROW_H,position:'relative',borderBottom:'1px solid #f1f5f9',background:isMatch?'#fefce8':i%2===0?'#fff':'#fafbfc'}} onDragOver={onRowDragOver} onDrop={e=>onRowDrop(e,w.id)}>
                        ...
                        {wUnavail.map((u,ui)=>(
                          <div key={ui}
                            ...
                            onContextMenu={e=>{e.preventDefault();setConfirm({msg:`"${u.reason}" 근무불가 일정을 삭제할까요?`,onOk:()=>delUnavail(u.id),okLabel:'삭제',okColor:'#dc2626'});}}
                          />
                        ))}
                        {/* 일정 블록 */}
                        {wSchs.map(sch=>{
                          ...
                          return(
                            <div key={sch.id}
                              ...
                              onClick={e=>{ if(isPast) onBlockClick(e,sch); }}
                              onContextMenu={e=>onRightClick(e,sch)}
                              style={{position:'absolute',left:(sch.startWd||0)*dayW+1,top:4,height:22,width:Math.max(6,(sch.durWd||1)*dayW-2),borderRadius:5,background:st.bg,border:`${curHit?2.5:1.5}px solid ${isSel||isHit?st.text:hasConflict?'#ef4444':st.border}`,cursor:isPast?'pointer':'grab',zIndex:isSel||curHit?5:3,display:'flex',alignItems:'center',paddingLeft:5,overflow:'hidden',boxShadow:isSel?`0 0 0 2px ${st.border},0 2px 6px rgba(0,0,0,0.1)`:hasConflict?'0 0 0 1.5px #ef4444':'0 1px 2px rgba(0,0,0,0.05)',opacity:isPast?0.7:1}}>
      ...
      {/* ★ 툴팁 */}
      {tooltip&&(
        <div style={{position:'fixed',left:tooltip.x+12,top:tooltip.y-8,background:'#1e293b',color:'#fff',padding:'4px 10px',borderRadius:6,fontSize:11,fontWeight:600,zIndex:4000,pointerEvents:'none',whiteSpace:'nowrap',boxShadow:'0 2px 8px rgba(0,0,0,0.25)'}}>
          {tooltip.text}
        </div>
      )}
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.3)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:2000}}>
          <div style={{background:'#fff',borderRadius:12,padding:'20px 22px',boxShadow:'0 8px 24px rgba(0,0,0,0.15)',width:300}}>
            ...
            <div style={{fontSize:11,color:'#64748b',marginBottom:10}}>
              {workers.find(w=>w.id===addUnavailWid)?.name} 관리사
            </div>
            <div style={{display:'flex',flexDirection:'column',gap:8}}>
              <div>
                <div style={{fontSize:10,color:'#94a3b8',fontWeight:700,marginBottom:2}}>시작일</div>
                <input type="date" value={newUnavail.startDate} onChange={e=>setNewUnavail(p=>({...p,startDate:e.target.value}))} style={{width:'100%',padding:'5px 8px',border:'1px solid #e2e8f0',borderRadius:6,fontSize:12}}/>
              </div>
              <div>
                <div style={{fontSize:10,color:'#94a3b8',fontWeight:700,marginBottom:2}}>종료일</div>
                <input type="date" value={newUnavail.endDate} onChange={e=>setNewUnavail(p=>({...p,endDate:e.target.value}))} style={{width:'100%',padding:'5px 8px',border:'1px solid #e2e8f0',borderRadius:6,fontSize:12}}/>
              </div>
              <div>
                <div style={{fontSize:10,color:'#94a3b8',fontWeight:700,marginBottom:2}}>사유</div>
                <input placeholder="예: 개인휴가, 교육, 병원" value={newUnavail.reason} onChange={e=>setNewUnavail(p=>({...p,reason:e.target.value}))} style={{width:'100%',padding:'5px 8px',border:'1px solid #e2e8f0',borderRadius:6,fontSize:12}}/>
              </div>
            </div>
            {/* 기존 근무불가 목록 */}
            {(unavailMap[addUnavailWid]||[]).length>0&&(
              <div style={{marginTop:10,padding:'6px 8px',background:'#fef2f2',borderRadius:6,border:'1px solid #fecaca'}}>
