...
// ── 날짜 정규화 (Google Sheets 다양한 형식 처리) ──────
const normDate = (v) => {
  try {
    if (!v && v !== 0) return '';
...
function buildWD() {
  ...
  if (!days[ti] || +days[ti] !== +TODAY) {
    for (let i = 0; i < days.length; i++) { if (+days[i] >= +TODAY) { ti = i; break; } }
  }
...
const dateToWd = (ds) => {
  try {
    const s = normDate(ds);
    if (!s) return TDX;
    const idx = WD.findIndex(d => dKey(d) === s);
  ...
  return (Array.isArray(raw)?raw:[]).map(s => {
    try {
      return autoStatus({
        ...
        memo: String(s.memo||''),
        expectedBirth: normDate(s.expectedBirth),  // ★ 날짜 정규화
        actualBirth:   normDate(s.actualBirth),    // ★ 날짜 정규화
        birthConfirm: toBool(s.birthConfirm),
        firstVisit:   toBool(s.firstVisit),
        midCheck:     toBool(s.midCheck),
        happycall:    toBool(s.happycall),
      });
    } catch(e) { return null; }
  }).filter(Boolean);
}
function parseWaiting(raw) {
  return (Array.isArray(raw)?raw:[]).map(w => {
    try { return { id:toInt(w.id,_uid++), name:String(w.name||''), expectedBirth:normDate(w.expectedBirth), durWd:Math.max(1,toInt(w.durWd,10)), memo:String(w.memo||'') }; }
    catch(e) { return null; }
  }).filter(Boolean);
}
...
export default function App() {
  ...
  const [panelOpen, setPanelOpen] = useState(true);  // ★ 왼쪽 패널 토글
  ...
  useEffect(() => {
    ...
    apiGet().then(data => {
      ...
      try {
        ...
        setWorkers(ws); setSchedules(ss); setWaiting(wt); setLoadState('ok');
  ...
  const triggerSave = useCallback((ws, ss, wt) => {
    ...
    saveTimer.current = setTimeout(async () => {
      try {
        const r = await apiSave({ workers:ws, schedules:ss.map(s=>({...s, startDate:wdToDate(s.startWd)})), waiting:wt });
  ...
  useEffect(() => {
    const calc = () => {
      ...
      setDayW(Math.max(16, Math.floor((wrapRef.current.offsetWidth - WORKER_W) / 44)));
    };
    calc();
    const ro = new ResizeObserver(calc);
    if (wrapRef.current) ro.observe(wrapRef.current);
    return () => ro.disconnect();
  }, [panelOpen]);  // ★ 패널 토글 시 재계산

  // ★ 초기 스크롤: 오늘에서 5평일(≈1주일) 전
  useEffect(() => {
  ...
  const onWScroll = useCallback(() => {
    ...
    if (syncV.current) return; syncV.current = true;
    ganttRef.current.scrollTop = workerRef.current.scrollTop;
    syncV.current = false;
  }, []);
  useEffect(() => {
  ...
  useEffect(() => {
    if (!sbDrag) return;
    const mv = (e) => {
      const sl = Math.max(0, Math.min(totalW-ganttVW, sbSL+((e.clientX-sbSX)/thumbMax)*(totalW-ganttVW)));
      if (ganttRef.current) ganttRef.current.scrollLeft = sl;
      if (dateHdrRef.current) dateHdrRef.current.scrollLeft = sl;
    };
  ...
  useEffect(() => {
    const h = (e) => { if (!e.target.closest('.ctx-menu')) setCtxMenu(null); };
  ...
  const activeWorkers = useMemo(() => {
    ...
    return [
      ...list.filter(w=>mw.has(w.id)),
      ...list.filter(w=>!mw.has(w.id)).sort((a,b)=>(a.group||1)!==(b.group||1)?(a.group||1)-(b.group||1):(a.name||'').localeCompare(b.name||'','ko')),
    ];
  }, [workers, schedules, q]);
  ...
  const sendToWaiting = (sch) => { setWait(p=>[...p,{id:_uid++,name:sch.client||'',expectedBirth:sch.expectedBirth||'',durWd:sch.durWd||10,memo:sch.memo||''}]); delSch(sch.id); };
  ...
  const onRowDrop = (e, wid) => {
    ...
    setSch(p=>[...p, autoStatus({id:_sid++,wid,client:wc.name||'',startWd:sw,durWd:wc.durWd||10,status:'hope',memo:wc.memo||'',expectedBirth:wc.expectedBirth||'',actualBirth:'',birthConfirm:false,firstVisit:false,midCheck:false,happycall:false})]);
  ...
  return (
    <div
      style={{display:'flex',flexDirection:'column',height:'100vh',fontFamily:"'Apple SD Gothic Neo','Malgun Gothic',sans-serif",background:'#f1f5f9',color:'#1e293b',overflow:'hidden'}}
    ...
    >
      ...
      <header style={{display:'flex',alignItems:'center',gap:8,padding:'0 12px',height:40,background:'#fff',borderBottom:'1px solid #e2e8f0',flexShrink:0,zIndex:20,boxShadow:'0 1px 4px rgba(0,0,0,0.06)'}}>
        ...
        <button
          ...
          title={panelOpen ? '왼쪽 패널 닫기' : '왼쪽 패널 열기'}
          style={{width:26,height:26,border:'1px solid #e2e8f0',borderRadius:6,background:panelOpen?'#eff6ff':'#f8fafc',color:panelOpen?'#3b82f6':'#94a3b8',cursor:'pointer',fontSize:13,flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',fontWeight:700}}
        >
          {panelOpen ? '◀' : '▶'}
        </button>
        ...
        {q && <div style={{display:'flex',alignItems:'center',gap:4,flexShrink:0}}>
          ...
          {searchHits.length>1 && <>
            <button onClick={()=>goHit(-1)} style={{padding:'2px 6px',borderRadius:5,border:'1px solid #93c5fd',background:'#eff6ff',color:'#1d4ed8',cursor:'pointer',fontSize:11}}>◀</button>
            <button onClick={()=>goHit(1)}  style={{padding:'2px 6px',borderRadius:5,border:'1px solid #93c5fd',background:'#eff6ff',color:'#1d4ed8',cursor:'pointer',fontSize:11}}>▶</button>
          </>}
        </div>}
        ...
        <div style={{display:'flex',gap:3,alignItems:'center',marginLeft:'auto',flexShrink:0,flexWrap:'wrap'}}>
          {Object.entries(STATUS_MAP).map(([k,v]) =>
            <span key={k} style={{padding:'1px 5px',borderRadius:20,fontSize:9,fontWeight:700,background:v.bg,border:`1.5px solid ${v.border}`,color:v.text,whiteSpace:'nowrap'}}>{v.label}</span>
          )}
        </div>
      </header>
      ...
      <div style={{display:'flex',flex:1,overflow:'hidden'}}>
        ...
        <div style={{
          ...
          background: '#fff',
          borderRight: panelOpen ? '1px solid #e2e8f0' : 'none',
          overflow: 'hidden',
          transition: 'width 0.2s ease',
        }}>
          {/* 일정 상세 */}
          <div style={{flexShrink:0,maxHeight:selSch?560:0,overflow:'hidden',transition:'max-height 0.25s ease',borderBottom:selSch?'2px solid #e2e8f0':'none'}}>
            {selSch && (
              <div className="scroll-y" style={{padding:'10px 12px',display:'flex',flexDirection:'column',gap:7,maxHeight:560}}>
                ...
                <div style={{padding:'7px 9px',borderRadius:8,background:STATUS_MAP[selSch.status]?.bg||'#f8fafc',border:`1.5px solid ${STATUS_MAP[selSch.status]?.border||'#94a3b8'}`}}>
                  ...
                  {editMode
                    ? <input value={selSch.client||''} onChange={e=>updSch({client:e.target.value})} style={iStyle({fontWeight:800,fontSize:13,width:'100%',marginBottom:2})}/>
                    : <div style={{fontWeight:800,fontSize:14}}>{selSch.client}</div>
                  }
                  <div style={{fontSize:10,color:'#64748b',marginTop:1}}>
                    {WD[selSch.startWd]?fmtD(WD[selSch.startWd]):'?'} ~ {WD[(selSch.startWd||0)+(selSch.durWd||0)-1]?fmtD(WD[(selSch.startWd||0)+(selSch.durWd||0)-1]):'?'} (평일 {selSch.durWd}일)
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div style={{display:'flex',flexWrap:'wrap',gap:3}}>
                    {Object.entries(STATUS_MAP).map(([k,v]) =>
                      <button key={k} onClick={()=>updSch({status:k})} style={{padding:'2px 5px',borderRadius:10,fontSize:9,fontWeight:700,cursor:'pointer',border:`1.5px solid ${v.border}`,background:selSch.status===k?v.border:v.bg,color:selSch.status===k?'#fff':v.text}}>{v.label}</button>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div style={{display:'flex',gap:3,alignItems:'center'}}>
                    {[10,15,20].map(n =>
                      <button key={n} onClick={()=>updSch({durWd:n})} style={{padding:'2px 7px',borderRadius:5,fontSize:10,fontWeight:700,cursor:'pointer',border:'1.5px solid #e2e8f0',background:selSch.durWd===n?'#3b82f6':'#f8fafc',color:selSch.durWd===n?'#fff':'#334155'}}>{n}일</button>
                    )}
                ...
                <div>
                  ...
                  {editMode
                    ? <textarea value={selSch.memo||''} onChange={e=>updSch({memo:e.target.value})} style={{width:'100%',height:48,border:'1px solid #c7d2fe',borderRadius:6,padding:'4px 6px',fontSize:10,resize:'none',fontFamily:'inherit',color:'#334155',outline:'none',background:'#f0f0ff',lineHeight:1.5}}/>
          ...
          <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',minHeight:0}}>
            ...
            {addWaitOpen && (
              <div style={{padding:'7px 10px',background:'#fffbeb',borderBottom:'1px solid #fde68a',display:'flex',flexDirection:'column',gap:4,flexShrink:0}}>
                ...
                <div style={{display:'flex',gap:4}}>
                  ...
                  <select value={newWait.durWd} onChange={e=>setNewWait(p=>({...p,durWd:+e.target.value}))} style={{width:52,padding:'3px 1px',border:'1px solid #fde68a',borderRadius:5,fontSize:10}}>
                    {[10,15,20].map(n=><option key={n} value={n}>{n}일</option>)}
                  </select>
                </div>
            ...
            <div className="scroll-y" style={{flex:1,padding:'5px 7px',minHeight:0}}>
              {waiting.length === 0
                ? <div style={{color:'#94a3b8',fontSize:10,textAlign:'center',padding:'16px 0'}}>대기 중인 산모 없음</div>
                : waiting.map(wc => wc && (
        ...
        <div ref={wrapRef} style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',minWidth:0}}>
          ...
          <div style={{display:'flex',height:DATE_H,flexShrink:0,zIndex:15,background:'#fff',borderBottom:'1px solid #e2e8f0',boxShadow:'0 2px 4px rgba(0,0,0,0.04)'}}>
            ...
            <div ref={dateHdrRef} style={{flex:1,overflowX:'hidden',overflowY:'hidden',background:'#f8fafc'}}>
              <div style={{width:TWD*dayW,display:'flex',height:'100%'}}>
                {WD.map((d,i) => {
                  const isT=i===TDX, lbl=dayLbl(i), isMon=lbl&&lbl.includes('월');
                  return <div key={i} style={{width:dayW,flexShrink:0,textAlign:'center',fontSize:9,fontWeight:isT||isMon?700:400,color:isT?'#2563eb':isMon?'#1e293b':'#94a3b8',borderRight:isMon?'1px solid #e2e8f0':'1px solid #f8fafc',background:isT?'#eff6ff':isMon?'#fafafa':'transparent',display:'flex',alignItems:'center',justifyContent:'center',position:'relative'}}>
                    {lbl||(isT?'▼':'')}
                    {isT&&<div style={{position:'absolute',bottom:0,left:'50%',transform:'translateX(-50%)',width:2,height:3,background:'#3b82f6',borderRadius:1}}/>}
                  </div>;
                })}
              </div>
            </div>
          ...
          {addWOpen && (
            <div style={{padding:'5px 9px',background:'#f0f9ff',borderBottom:'1px solid #bae6fd',display:'flex',gap:5,alignItems:'center',flexShrink:0,flexWrap:'wrap'}}>
              ...
              <select value={newW.group} onChange={e=>setNewW(p=>({...p,group:+e.target.value}))} style={{padding:'3px 3px',border:'1px solid #bae6fd',borderRadius:5,fontSize:11}}>
                {GROUPS.map(g=><option key={g.id} value={g.id}>G{g.id} {g.label}</option>)}
              </select>
          ...
          <div style={{display:'flex',flex:1,overflow:'hidden',minHeight:0}}>
            ...
            {/* ★ 관리사 열 - 세로 스크롤 활성화 */}
            <div ref={workerRef} onScroll={onWScroll}
              className="scroll-y"
              style={{width:WORKER_W,flexShrink:0,background:'#fff',borderRight:'1px solid #e2e8f0',overflowX:'hidden'}}>
              {visibleW.map((w,i) => {
                if (!w) return null;
                ...
                return (
                  <div key={w.id}>
                    ...
                    <div className="wrow" style={{height:ROW_H,display:'flex',alignItems:'center',padding:'0 5px',borderBottom:'1px solid #f1f5f9',background:isMatch?'#fef9c3':i%2===0?g.bgRow:g.bgAlt,gap:3,position:'relative'}} onMouseEnter={()=>setHoverWId(w.id)} onMouseLeave={()=>setHoverWId(null)}>
                      {isEdit ? (
                        <>
                          <input value={editWVal.name} onChange={e=>setEditWVal(p=>({...p,name:e.target.value}))} style={{width:42,padding:'2px 3px',border:'1px solid #93c5fd',borderRadius:4,fontSize:10}}/>
                          <input value={editWVal.area} onChange={e=>setEditWVal(p=>({...p,area:e.target.value}))} style={{flex:1,padding:'2px 3px',border:'1px solid #93c5fd',borderRadius:4,fontSize:9,minWidth:0}}/>
                          <select value={editWVal.group} onChange={e=>setEditWVal(p=>({...p,group:+e.target.value}))} style={{width:22,padding:'1px',border:'1px solid #93c5fd',borderRadius:3,fontSize:9}}>{[1,2,3,4].map(n=><option key={n} value={n}>{n}</option>)}</select>
                          <button onClick={()=>saveEdit(w.id)} style={{padding:'1px 3px',borderRadius:3,background:'#3b82f6',color:'#fff',border:'none',cursor:'pointer',fontSize:9}}>✓</button>
                          <button onClick={()=>setEditWId(null)} style={{padding:'1px 3px',borderRadius:3,background:'#e2e8f0',color:'#475569',border:'none',cursor:'pointer',fontSize:9}}>✗</button>
                        </>
                      ) : (
                        <>
                          ...
                          {hoverWId===w.id && (
                            <div className="wact" style={{display:'flex',gap:2,flexShrink:0}}>
              ...
              {inactiveW.length>0 && (
                <button onClick={()=>setInactiveOpen(v=>!v)} style={{width:'100%',height:24,display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 10px',background:'#f1f5f9',border:'none',borderTop:'2px dashed #cbd5e1',cursor:'pointer',fontSize:10,fontWeight:700,color:'#64748b'}}>
                  <span>📁 비활성 ({inactiveW.length}명)</span><span>{inactiveOpen?'▲':'▼'}</span>
                </button>
              )}
            </div>

            {/* ★ 간트 본문 - 세로/가로 스크롤 활성화 */}
            <div ref={ganttRef} onScroll={onGScroll}
              className="scroll-xy"
              style={{flex:1,position:'relative',cursor:dragging?'grabbing':'default'}}>
                ...
                {visibleW.map((w,i) => {
                  ...
                  return (
                    <div key={w.id}>
                      ...
                      <div style={{height:ROW_H,position:'relative',borderBottom:'1px solid #f1f5f9',background:isMatch?'#fefce8':i%2===0?'#fff':'#fafbfc'}} onDragOver={onRowDragOver} onDrop={e=>onRowDrop(e,w.id)}>
                        ...
                        {wSchs.map(sch => {
                          ...
                          return (
                            <div key={sch.id}
                              onMouseDown={e=>onBlockDown(e,sch)}
                              onContextMenu={e=>onRightClick(e,sch)}
                              style={{position:'absolute',left:(sch.startWd||0)*dayW+1,top:4,height:22,width:Math.max(6,(sch.durWd||1)*dayW-2),borderRadius:5,background:st.bg,border:`${curHit?2.5:1.5}px solid ${isSel||isHit?st.text:st.border}`,cursor:dragging===sch.id?'grabbing':'grab',zIndex:isSel||curHit?5:3,display:'flex',alignItems:'center',paddingLeft:5,overflow:'hidden',boxShadow:isSel?`0 0 0 2px ${st.border},0 2px 6px rgba(0,0,0,0.1)`:curHit?`0 0 0 2.5px ${st.text}`:'0 1px 2px rgba(0,0,0,0.05)'}}>
      ...
      {/* 우클릭 메뉴 */}
      {ctxMenu && (
        <div className="ctx-menu" style={{position:'fixed',left:ctxMenu.x,top:ctxMenu.y,background:'#fff',border:'1px solid #e2e8f0',borderRadius:8,boxShadow:'0 4px 16px rgba(0,0,0,0.12)',zIndex:1000,minWidth:150,overflow:'hidden'}}>
          ...
          {[10,15,20].map(n => {
            const cur=schedules.find(s=>s&&s.id===ctxMenu.sid)?.durWd;
            return <button key={n} onClick={()=>setDur(ctxMenu.sid,n)} style={{display:'block',width:'100%',padding:'7px 14px',border:'none',background:cur===n?'#eff6ff':'#fff',textAlign:'left',fontSize:12,fontWeight:cur===n?700:400,color:cur===n?'#2563eb':'#334155',cursor:'pointer',borderBottom:'1px solid #f8fafc'}}>{n}일 {cur===n?'✓':''}</button>;
          })}
      ...
      {/* 확인창 */}
      {confirm && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.28)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:2000}}>
